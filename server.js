...
app.post('/answer', async (req, res) => {
    ...
    try {
        // Call the Groq API to generate an answer based on the question
        const temp = await groq.chat.completions.create({
            messages: [
                {
                    role: "user",
                    content: question+" answer in 100 to 200 words",
                },
            ],
            model: "llama3-8b-8192", // Use the same model as before
            // max_tokens: 150, // Limit the response length to 150 tokens (approximately 100-200 words)
        });

        const answer = temp.choices[0].message.content;
        res.json({ answer });
    } catch (error) {
